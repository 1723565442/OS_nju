...
LABEL_START:			; <--- 从这里开始 *************
	...
	mov	ds, ax
	mov	es, ax
	mov	ss, ax	;cs的值赋值给ds, es, ss,使得内核和程序在同一段内存中
	mov	sp, BaseOfStack
...
.MemChkLoop:
	mov	eax, 0E820h		; eax = 0000E820h   e820功能
	mov	ecx, 20			; ecx = 地址范围描述符结构的大小
	mov	edx, 0534D4150h		; edx = 'SMAP' 表示查询内存类型信息
	int	15h			; int 15h
	jc	.MemChkFail
	add	di, 20
	inc	dword [_dwMCRNumber]	; dwMCRNumber = ARDS 的个数
	cmp	ebx, 0 ;ebx 作为记录查询的返回值
	jne	.MemChkLoop
	jmp	.MemChkOK
.MemChkFail:
...
.MemChkOK:
	...
	; 下面在 A 盘的根目录寻找 KERNEL.BIN的准备工作
	mov	word [wSectorNo], SectorNoOfRootDirectory	
	xor	ah, ah	; ┓
	xor	dl, dl	; ┣ 软驱复位
	int	13h	; ┛ 将软驱的磁头 柱面 扇区数都设置为0 保证下次读取从首个扇区开始 接下来就是读取软盘山区的数据也就是吧文件加载进内存
...
LABEL_SEARCH_IN_ROOT_DIR_BEGIN:
	...
	;于是, es:bx = BaseOfKernelFile:OffsetOfKernelFile 指向内核文件的实际地址
	mov	ax, [wSectorNo]		; ax <- Root Directory 中的某 Sector 号
	mov	cl, 1 ;读取一个扇区 读到的扇区中的数据存储在es：bx指向的内存地址中
	call	ReadSector

	mov	si, KernelFileName	; ds:si -> "KERNEL  BIN"
	mov	di, OffsetOfKernelFile	; es:di -> BaseOfKernelFile:???? = BaseOfKernelFile*10h+????
	cld ; 将方向标志位DF清零 保证窜操作的方向为向前递增
	mov	dx, 10h; 把偏移地址的长度10H存储在dx寄存器中，为后面扫描文件名的循环做好准备
...
LABEL_DIFFERENT:
	...
	add	di, 20h			;     ┃
	mov	si, KernelFileName	;     ┣ di += 20h  下一个目录条目 即下一个文件名
...
LABEL_FILENAME_FOUND:			; 找到 KERNEL.BIN 后便来到这里继续
	mov	ax, RootDirSectors ; 根目录占用空间
	and	di, 0FFF0h		; di -> 当前条目的开始  每个条目在16字节边界上对其

	push	eax
	mov	eax, [es : di + 01Ch]	;目录项的27-30字节的内容 也就是文件的大小	; ┓
	mov	dword [dwKernelSize], eax	; ┛保存 KERNEL.BIN 文件大小
	pop	eax

	add	di, 01Ah		; di -> 文件数据所在的首 Sector
	mov	cx, word [es:di]
...
; 真正进入保护模式
	...
	;将控制传递给在“SelectorFlatC”段中实现保护模式代码的第一个指令或标签
...
; 显示内存信息 --------------------------------------------------------------
; 显示内存的地址范围的基地址低 基地址高 长度低 长度高 类型
DispMemInfo:
	push	esi
	push	edi
	push	ecx
...
; 启动分页机制 --------------------------------------------------------------
; 即 地址转换 ： 虚拟地址--》 物理地址 映射到物理内存
SetupPaging:
	; 根据内存大小计算应初始化多少PDE以及多少页表
	xor	edx, edx
	mov	eax, [dwMemSize]
	mov	ebx, 400000h	; 400000h = 4M = 4096 * 1024, 一个页表对应的内存大小
	div	ebx 		; dwMemSize / ebx
...
.no_remainder:
	...
	; 首先初始化页目录
	mov	ax, SelectorFlatRW	;页目录表的段选择器值
	mov	es, ax
	mov	edi, PageDirBase	; 此段首地址为 PageDirBase
	xor	eax, eax
	mov	eax, PageTblBase | PG_P  | PG_USU | PG_RWW  ;页表起始地址 存在标记 用户空间标记 读写标记
.1:
	stosd
