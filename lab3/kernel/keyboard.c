...
PRIVATE KB_INPUT	kb_in; //存储从键盘输入的扫描马
...
/*======================================================================*
                            keyboard_handler
 *======================================================================*/
PUBLIC void keyboard_handler(int irq) //键盘中断处理 将扫描码存储到缓冲区（队列）
{
...
/*======================================================================*
                           keyboard_read
*======================================================================*/
PUBLIC void keyboard_read(TTY* p_tty)//从输入缓冲区读取扫描码 并对按键解析
{
	u8	scan_code;
	char	output[2];
	...
	if(kb_in.count > 0){
		...
		if (scan_code == 0xE1) {
			...
			例如  箭头键、功能键F1-F12、左右Shift、Ctrl和Alt  */
			if (key == 0) {
				code_with_E0 = 1;
			}
		}
...
PRIVATE void kb_wait()	/* 等待 8042 的输入缓冲区空 */
{
	...
	do {
		kb_stat = in_byte(KB_CMD); //获取键盘状态信息 并和0010与 来判断是否空闲
	} while (kb_stat & 0x02);
}
...
/*======================================================================*
				 kb_ack
 *======================================================================*/
PRIVATE void kb_ack() //等待一个键盘确认信号
{
	u8 kb_read;
